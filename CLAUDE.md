...
| Un suivi arborescent | `.arbo > .arb.nN (.arb-main + .pbadge + .arb-prog (.meter + .arb-pct) + .arb-time + .arb-date)` — `n1` l'usager, `n2` le contenu, `n3` sa séquence. Trois niveaux, pas plus : un niveau qui ne porte qu'un mot et un total redevient une pastille sur la ligne du dessous et un décompte sur celle du dessus. `arboPlie(this)` sur le `.arb-tog` replie la branche, `.arb-vide` garde la place du chevron sans enfant, `.arb.arb-hd` nomme les colonnes et se fige en haut |
