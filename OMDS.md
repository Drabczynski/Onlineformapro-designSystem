...
| Un suivi arborescent | `.arbo > .arb.nN (.arb-main + .meter + .arb-pct + .arb-time)` — `n1` l'usager, `n2` la nature du contenu, `n3` le contenu, `n4` sa séquence. Un rail vertical donne la profondeur, `arboPlie(this)` sur le `.arb-tog` replie toute la branche, `.arb-vide` garde la place du chevron quand la ligne n'a pas d'enfant. `.arb.arb-hd` nomme les colonnes et se fige en haut |
